...
func (a *API) CreateNewChirp(w http.ResponseWriter, r *http.Request) {
	msg := messageReq{}
	...
	_, err := a.db.GetUserById(r.Context(), userId)
	if err != nil {
	...
	chirp, err := a.db.CreateChirp(r.Context(), database.CreateChirpParams{Body: filteredBody, UserID: userId})
	if err != nil {
